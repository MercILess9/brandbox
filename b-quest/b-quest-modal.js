/**
 * B-QUEST MODAL COMPONENT - THE ULTIMATE MASTER (FIXED SEARCH)
 */
...
<style>
    ...
    /* Search Overlay UI (ลอยออกมาสวยๆ) */
    .bq-search-overlay { 
    ...
    .bq-search-card { 
        ...
        flex-direction: column; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25);
    }
    .uni-item-modern { 
            ...
            <div id="bq-search-overlay" class="bq-search-overlay" onclick="closeSearchOverlay()">
                <div class="bq-search-card" onclick="event.stopPropagation()">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h5 class="fw-800 m-0" style="color:#1e293b">Selection</h5>
                        <button type="button" class="btn-close" onclick="closeSearchOverlay()"></button>
                    </div>
                    <div class="position-relative mb-3">
                        <i class="bi bi-search position-absolute" style="left:15px; top:50%; transform:translateY(-50%); color:#94a3b8"></i>
                        <input type="text" class="form-control" id="uni-search-input" placeholder="Search data..." style="padding-left:40px; border-radius:15px; height:45px; border:1px solid #e2e8f0">
                    </div>
...
async function openSearchOverlay(fieldName, targetId) {
    ...
    container.innerHTML = '<div class="p-4 text-center text-muted">Loading data...</div>';
    searchInput.value = '';

    try {
        const { data, error } = await supabaseClient.from('b-quest-list').select(fieldName);
        ...
        const renderItems = (filter = '') => {
            ...
            if (filtered.length === 0) { container.innerHTML = '<div class="p-3 text-center text-muted">No data found</div>'; return; }
    ...
    } catch (e) { container.innerHTML = '<div class="p-3 text-center text-danger">Error fetching data</div>'; }
}
...
function updateRoleUI(role) {
    ...
    if (checkbox.checked) { card.classList.add('active'); card.classList.remove('disabled'); els.forEach(el => el.required = true); } 
