/**
 * B-QUEST MODAL - THE PROFESSIONAL RESTORED VERSION
 */
...
<style>
    ...
    .bq-glass-card { background: #ffffff; border-radius: 22px; padding: 22px; border: 1px solid #e2e8f0; height: 100%; display: flex; flex-direction: column; }
    
    .bq-label-modern { font-size: 0.65rem; font-weight: 800; color: #94a3b8; margin-bottom: 5px; text-transform: uppercase; letter-spacing: 0.8px; }
    .bq-input-modern { 
        width: 100%; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; 
        padding: 8px 15px; font-size: 0.88rem; color: #334155; margin-bottom: 12px; 
        text-align-last: center; transition: 0.2s;
    }
    .bq-input-modern:focus { border-color: #bdc432; outline: none; box-shadow: 0 0 0 4px rgba(189, 196, 50, 0.1); }
    .bq-input-detail { flex-grow: 1; min-height: 145px; text-align: left !important; text-align-last: left !important; resize: none; }

    /* --- Role Cards --- */
    .role-card { background: #fff; border-radius: 24px; border: 1px solid #e2e8f0; margin-bottom: 15px; transition: all 0.4s ease; overflow: hidden; }
    .role-card.disabled { opacity: 0.5; background: #f1f5f9; }
    .role-card-header { padding: 18px 22px; display: flex; align-items: center; gap: 12px; }
    .role-card-title { font-size: 0.9rem; font-weight: 800; color: #1e293b; margin: 0; }
    .role-card-title i { font-size: 1.1rem; color: #94a3b8; }
    .role-card.active .role-card-title i { color: #1e293b; }
    
    .bq-assign-badge { background: #eff6ff; color: #3b82f6; padding: 3px 12px; border-radius: 10px; font-size: 0.75rem; font-weight: 800; border: 1px solid #dbeafe; display: none; }

    .role-card-body { max-height: 0; padding: 0 22px; transition: all 0.4s ease; visibility: hidden; opacity: 0; }
    .role-card.active .role-card-body { max-height: 500px; padding: 20px 22px 22px 22px; border-top: 1px solid #f1f5f9; visibility: visible; opacity: 1; }

    /* --- Status & Capacity --- */
    .bq-status-select { border: 1px solid #e2e8f0; border-radius: 10px; font-size: 0.75rem; font-weight: 800; padding: 4px 10px; color: #fff; min-width: 100px; text-align-last: center; }
    ...
    .timeline-zone { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 18px; padding: 15px; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; }
    .bq-cap-text { font-size: 0.9rem; font-weight: 800; margin-top: 10px; display: none; text-align: center; }

    /* --- 🌟 🌟 🌟 Search Overlay (Beautiful Floating Card) --- */
    .bq-search-overlay { 
    ...
    .bq-search-card { 
        background: #fff; width: 480px; max-height: 80vh; 
        border-radius: 35px; padding: 30px; display: flex; 
        flex-direction: column; box-shadow: 0 30px 60px -12px rgba(0,0,0,0.3);
    }
    .uni-item-modern { 
        border: none; background: #fff; border-radius: 15px; 
        margin-bottom: 8px; padding: 15px 20px; font-size: 0.95rem; 
        font-weight: 600; text-align: left; cursor: pointer; transition: 0.2s;
        border: 1px solid #f1f5f9;
    }
    .uni-item-modern:hover { background: #f8fafc; border-color: #bdc432; color: #bdc432; padding-left: 25px; }

    /* --- Buttons --- */
    .bq-footer-actions { padding: 20px 35px; display: flex; justify-content: flex-end; gap: 15px; background: #fff; border-top: 1px solid rgba(0,0,0,0.05); }
    .btn-bq-delete { background: #fee2e2; color: #ef4444; border: none; padding: 0 25px; border-radius: 14px; font-weight: 700; height: 45px; display: none; cursor: pointer; }
    .btn-bq-create { background: #1e293b; color: #bdc432; border: none; padding: 0 40px; border-radius: 14px; font-weight: 700; height: 45px; cursor: pointer; }

    /* Toggle Switch */
    .bq-toggle { position: relative; display: inline-block; width: 38px; height: 20px; }
    ...
    .bq-slider:before { position: absolute; content: ""; height: 14px; width: 14px; left: 3px; bottom: 3px; background-color: white; transition: .4s; border-radius: 50%; }
            ...
            <div id="bq-search-overlay" class="bq-search-overlay" onclick="closeSearchOverlay()">
                <div class="bq-search-card" onclick="event.stopPropagation()">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h4 class="fw-800 m-0" style="color:#1e293b">Select Data</h4>
                        <button type="button" class="btn-close" onclick="closeSearchOverlay()"></button>
                    </div>
                    <div class="position-relative mb-3">
                        <i class="bi bi-search position-absolute" style="left:18px; top:50%; transform:translateY(-50%); color:#94a3b8"></i>
                        <input type="text" class="form-control" id="uni-search-input" placeholder="Type to search..." style="padding-left:45px; border-radius:18px; height:50px; border:1px solid #e2e8f0; font-weight:600;">
                    </div>
                        ...
                        <div class="col-lg-6">
                            <div class="bq-glass-card">
                                ...
                                <div class="d-flex mb-1">
                                    <input type="text" class="bq-input-modern m-0" style="border-radius:12px 0 0 12px; text-align-last:left;" id="b-quest-modal-account" name="account_name" required>
                                    <button type="button" class="btn border border-start-0" style="border-radius:0 12px 12px 0; background:#fff; height:42px; width:50px;" onclick="openSearchOverlay('account_name', 'b-quest-modal-account')"><i class="bi bi-search"></i></button>
                                </div>
                                <label class="bq-label-modern">Opportunity Name</label>
                                <div class="d-flex mb-1">
                                    <input type="text" class="bq-input-modern m-0" style="border-radius:12px 0 0 12px; text-align-last:left;" id="b-quest-modal-opportunity" name="opportunity_name" required>
                                    <button type="button" class="btn border border-start-0" style="border-radius:0 12px 12px 0; background:#fff; height:42px; width:50px;" onclick="openSearchOverlay('opportunity_name', 'b-quest-modal-opportunity')"><i class="bi bi-search"></i></button>
                                </div>
                                ...
                                <div class="row g-2">
                                    <div class="col-8">
                                        ...
                                        <input type="text" class="bq-input-modern" style="text-align-last:left;" id="b-quest-modal-link" name="link">
                                    </div>
                                    <div class="col-4">
                                        <label class="bq-label-modern text-center d-block">Publish Date</label>
                                        <input type="date" class="bq-input-modern" id="b-quest-modal-publish-date" name="publish_date" required>
                                    </div>
                                </div>
                                <label class="bq-label-modern">Detail</label>
                                <textarea class="bq-input-modern bq-input-detail" id="b-quest-modal-detail" name="detail"></textarea>
                            </div>
                        </div>
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-6">
                                            <div class="timeline-zone">
                                                ...
                                                <span class="bq-cap-text" id="designer-capacity-info"></span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-6">
                                            <div class="timeline-zone">
                                                ...
                                                <span class="bq-cap-text" id="creative-capacity-info"></span>
                                            </div>
                                        </div>
                                    </div>
...
async function handleDeleteTask() {
    ...
    const res = await Swal.fire({ title: 'Are you sure?', text: "You won't be able to revert this!", icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444' });
...
document.getElementById('b-quest-modal-form').addEventListener('submit', async (e) => {
    ...
    if (!isDes) { payload.designer = null; payload.designer_type = null; payload.designer_deadline = null; payload.designer_weight = 0; payload.designer_assign = null; payload.designer_status = null; }
    if (!isCre) { payload.creative = null; payload.creative_type = null; payload.creative_deadline = null; payload.creative_weight = 0; payload.creative_assign = null; payload.creative_status = null; }
