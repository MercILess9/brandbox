/**
 * B-QUEST MODAL COMPONENT - THE MASTER FINAL V4
 * -----------------------------------------------------------
 * - Smart UI: Hide Status/Assign for New Task or New Role addition
 * - Auto-Status: Set to 'Progress' automatically on Save/Create
 * - Data Integrity: Clean to NULL when Switch is OFF
 */
...
function updateRoleUI(role) {
    ...
    if (cb.checked) {
        ...
        // 🚩 เมื่อปิด Card ให้ซ่อน Status และ Assign เสมอ
    ...
    if (taskId) {
        ...
        if (data) {
            ...
            ['designer', 'creative'].forEach(role => {
                ...
                // 🚩 โชว์ Status เฉพาะกรณีที่เปิด Card นี้อยู่ และมี Assign แล้ว
                statusEl.style.display = (hasRoleData && hasAssign) ? 'block' : 'none';
...
function fillFormData(data) {
    ...
    ['designer', 'creative'].forEach(role => {
        ...
        // 🚩 โชว์ป้ายชื่อเฉพาะกรณีที่มีคนรับงานแล้ว
...
document.getElementById('b-quest-modal-form').addEventListener('submit', async (e) => {
    ...
    // 🚩 ล้างข้อมูลฝั่งที่ปิด Switch ให้เป็น NULL และจัดการ Auto-Status
    if (!isDes) {
        ...
        // หากเป็นงานใหม่ หรือเปิด Card ครั้งแรก ให้ Status เป็น Progress อัตโนมัติ
